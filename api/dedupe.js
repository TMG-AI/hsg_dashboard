...
const SEEN_LINK = "mentions:seen:link";
...
export default async function handler(req, res) {
  try {
    // Pull a generous window (adjust as needed)
    const raw = await r.zrange(ZSET, 0, 1000, { rev: true });
    const items = raw.map(toObj).filter(Boolean);

    const keep = new Map(); // canon -> newest item
    const drop = [];        // members to delete

    for (const m of items) {
      const canon = m.canon || normalizeUrl(m.link || "");
      if (!canon) continue;
      const key = canon;
      if (!keep.has(key)) { keep.set(key, m); }
      else {
        // existing is newer (because we read newest-first), so drop this older one
        drop.push(m);
      }
    }

    // Remove dropped members from ZSET and SEEN sets
    let removed = 0;
    for (const m of drop) {
      try {
        await r.zrem(ZSET, JSON.stringify(m)); // member is the JSON we stored
        await r.srem(SEEN, m.id);
        const canon = m.canon || normalizeUrl(m.link || "");
