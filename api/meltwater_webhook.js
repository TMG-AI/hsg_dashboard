...
function normalizeUrl(u){ try{const url=new URL(u); url.hash="";
  ...
  let s=url.toString(); if (s.endsWith("/")) s=s.slice(0,-1); return s; } catch { return (u||"").trim(); } }
...
export default async function handler(req, res){
  try{
    ...
    // Meltwater alert payloads vary; handle a few common shapes:
