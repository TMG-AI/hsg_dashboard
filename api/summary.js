...
function toTs(m){
  if (Number.isFinite(m?.published_ts)) return m.published_ts;
...
function detectOrigin(m){
  ...
  try {
    ...
    // Google Alerts patterns: news.google.*, feedproxy.google.*, www.google.com/url?… (GA redirects)
...
export default async function handler(req, res){
  try{
    ...
    // ✅ Read the most recent N items by index (works even if ZSET scores aren't "today")
    const LOOKBACK = 2000; // increase if you ingest more than this/day
    const raw = await redis.zrange(ZSET, -LOOKBACK, -1, { rev: true });
    const all = raw.map(safeParse).filter(Boolean);

    // 1) Keep only today's items by each item's own timestamps
    const today = all.filter(m => toTs(m) >= startToday);

    // 2) Drop the mock/test items you showed
    const clean = today.filter(m => !isMock(m));

    // 3) Tally by origin (Meltwater / Google Alerts / RSS; others kept if present)
