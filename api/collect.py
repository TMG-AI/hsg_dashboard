...
# --- NEW: Urgent Keywords for high-signal alerts ---
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not KEYWORDS:
            self.send_response(400); self.end_headers(); self.wfile.write(b"Configuration Error: Missing KEYWORDS."); return
        ...
        for url in RSS_FEEDS:
            try:
                ...
                for entry in feed.entries:
                    ...
                    # First, check if it's a relevant mention to save to the dashboard
                    if any(keyword in text_to_search for keyword in KEYWORDS):
                        mention_id = get_mention_id(entry.link)
                        
                        # Only process if it's a brand new mention
                        if not redis_client.exists(mention_id):
                            ...
                            # Always save all relevant mentions to the database for the dashboard
                            store_mention_data(mention_id, mention_data)

                            # --- NEW LOGIC: Only send an alert if it's URGENT ---
