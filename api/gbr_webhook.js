// /api/gbr_webhook.js
// Webhook for GBR Google Alerts - tags with origin: "gbr"
...
function idFromCanonical(canon){ let h=0; for (let i=0;i<canon.length;i++) h=(h*31+canon.charCodeAt(i))>>>0; return `gbr_${h.toString(16)}`; }
...
export default async function handler(req, res){
  try{
    ...
    if (process.env.GBR_WEBHOOK_SECRET) {
      const urlObj = new URL(req.url, "http://localhost");
      const key = (urlObj.searchParams.get("key")||"").trim();
      if (!key || key !== process.env.GBR_WEBHOOK_SECRET) return res.status(401).send("bad secret");
    }
    ...
    console.log(`[GBR] Stored: "${title}" from ${mention.source}`);

    res.status(200).json({ ok:true, stored:1, sample:{ title, link } });
  }catch(e){
    console.error('GBR webhook error:', e);
